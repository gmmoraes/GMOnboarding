...
public struct ConcentricOnboardingView<Content>: View, Animatable where Content: View {
    ...
    private var backButton: some View {
        ...
        return Button(action: backTapAction) {
        ...
        .offset(x: isAnimated ? (coeff * maxXOffset) - 150 : -150,
                y: isAnimated ? maxYOffset - 200 : -200)
        .opacity(isFirstIndex() ? 0 : 1)
    }
    ...
    private var button: some View {
        ZStack {
            ...
            .disabled(isAnimated || isLastIndex())
        }
        .offset(y: 300)
//        .opacity(isLastIndex() ? 0 : 1)
    ...
    private func updateColors(forNextPage: Bool = false) {
        ...
        let circleCurrentIndex = min(pageContents.count - 1, nextIndex + 1) // Index da Cor do botão
        let buttonColor = pageContents[circleCurrentIndex].background
        let animationColor = pageContents[nextIndex].background
        circleColor = forNextPage ? buttonColor : animationColor//pageContents[forNextPage ? circleCurrentIndex : nextIndex].background
    }
    ...
    private func isLastIndex() -> Bool {
        currentIndex == (pageContents.count - 1)
    }
}
